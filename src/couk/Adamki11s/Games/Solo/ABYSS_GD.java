...
public class ABYSS_GD extends Gamedata {
	...
	private Location sign1 = new Location(Maps.Warzone_World, -157.5D, 88.620000000000005D, 559.5D);
	
	private Location sign2 = new Location(Maps.Warzone_World, -182.5D, 88.620000000000005D, 537.5D);
	...
	public void tickerTask(int schedulerTask) {
		Player quitter = null, stayer = null;
		...
		if(!playerQuit){
			super.updateGUIWrapper(participants.get(0), participants.get(1), (gameTime - timer), score.get(participants.get(0)), score.get(participants.get(1)));
		} else {
			for(Player p : participants){
				if(spoutData.doesPlayerHaveSpout(p)){
					super.clearWidgets((SpoutPlayer)stayer);
				}
			}
		}
	}
